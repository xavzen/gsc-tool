...
struct node_stmt_for : public node
{
    ...
    node_stmt_for(const location& loc, expr_ptr pre_expr, expr_ptr expr, expr_ptr post_expr, stmt_ptr stmt)
...
struct node_stmt_foreach : public node
{
    ...
    node_stmt_foreach(name_ptr key, name_ptr element, expr_ptr container, stmt_ptr stmt)
    ...
    node_stmt_foreach(const location& loc, name_ptr element, expr_ptr container, stmt_ptr stmt)
        : node(node_t::stmt_foreach, loc), element(std::move(element)), container(std::move(container)),
            stmt(std::move(stmt)), use_key(false) {}

    node_stmt_foreach(const location& loc, name_ptr key, name_ptr element, expr_ptr container, stmt_ptr stmt)
