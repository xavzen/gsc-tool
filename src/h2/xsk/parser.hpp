...
/**
 ** \file parser.hpp
 ** Define the xsk::gsc::h2::parser class.
 */
...
# endif
    ...
    {
      H2_ASSERT (yytypeid_);
      H2_ASSERT (*yytypeid_ == *that.yytypeid_);
...
switch (yykind)
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
...
#endif
      {
        H2_ASSERT (tok == token::H2EOF
                   || (token::H2error <= tok && tok <= token::DIV)
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      static
      symbol_type
      make_H2EOF (location_type l)
      {
        return symbol_type (token::H2EOF, std::move (l));
      }
#else
      static
      symbol_type
      make_H2EOF (const location_type& l)
      {
        return symbol_type (token::H2EOF, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_H2error (location_type l)
      {
        return symbol_type (token::H2error, std::move (l));
      }
#else
      static
      symbol_type
      make_H2error (const location_type& l)
      {
        return symbol_type (token::H2error, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_H2UNDEF (location_type l)
      {
        return symbol_type (token::H2UNDEF, std::move (l));
      }
#else
      static
      symbol_type
      make_H2UNDEF (const location_type& l)
      {
...
#endif // !YY_H2_PARSER_HPP_INCLUDED
