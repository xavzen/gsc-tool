...
auto compiler::parse_buffer(const std::string& file, std::vector<std::uint8_t>& data) -> gsc::program_ptr
{
    ...
    if (iw7_lex_init(&scanner))
        exit(1);

    YY_BUFFER_STATE yybuffer = iw7__scan_buffer(reinterpret_cast<char*>(data.data()), data.size(), scanner);
    ...
    iw7__delete_buffer(yybuffer, scanner);
    iw7_lex_destroy(scanner);
