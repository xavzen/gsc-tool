...
auto compiler::parse_buffer(const std::string& file, std::vector<std::uint8_t>& data) -> gsc::program_ptr
{
    ...
    if (s2_lex_init(&scanner))
        exit(1);

    YY_BUFFER_STATE yybuffer = s2__scan_buffer(reinterpret_cast<char*>(data.data()), data.size(), scanner);
    ...
    s2__delete_buffer(yybuffer, scanner);
    s2_lex_destroy(scanner);
