...
auto compiler::parse_buffer(std::vector<std::uint8_t>& data) -> gsc::program_ptr
{
    ...
    if (H1lex_init(&scanner))
        exit(1);

    YY_BUFFER_STATE yybuffer = H1_scan_buffer(reinterpret_cast<char*>(data.data()), data.size(), scanner);
    ...
    H1_delete_buffer(yybuffer, scanner);
    H1lex_destroy(scanner);
